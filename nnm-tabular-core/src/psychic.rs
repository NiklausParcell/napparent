//! Psychic Barnacle pair aggregation (`PsychicBarnacle` in Python).
    ...
    pub fn vals_map_updating(
        ...
        for &c in &self.col_array {
            ...
            for (v0, v1) in frequent {
                ...
                self.vals_map
                    .entry(unique_tup)
                    .or_insert([0.0, 0.0])[0] += sum_direct;
                self.vals_map
                    .entry(unique_inv_tup)
                    .or_insert([0.0, 0.0])[0] += sum_inv;
    ...
    pub fn use_map(
        ...
        for c_idx in 0..self.col_graph_names.len() {
            if let Some(arr) = col_combined.get(&c_idx) {
                ...
                nnm.insert(format!("{col_name}_barn"), ColumnVec::F32(diffs));
            }
        }

        nnm.insert("Actuals".into(), ColumnVec::F32(y.to_vec()));
        nnm.insert("outcomes_barn".into(), ColumnVec::F32(outcomes.to_vec()));
        Ok(nnm)
    }
}

impl Default for PsychicBarnacle {
    fn default() -> Self {
        Self::new()
    }
}
