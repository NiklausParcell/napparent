...
impl PreprocessStream {
    ...
    pub fn finish_map(&mut self, depth: &BarnacleDepth) -> Result<(), String> {
        for (&col, cp) in self.preprocess_map.iter_mut() {
            let barn_depth = depth.depth_for(col);
            cp.values.remove(&ValueKey::Str("nan".into()));

            let items: Vec<(ValueKey, u64)> = cp.values.iter().map(|(k, v)| (k.clone(), *v)).collect();
            let selected = return_accounted_bins(&items, barn_depth);
            ...
            bins_sorted.sort_by(|a, b| key_cmp_for_sort(a, b));
            ...
            match cp.bin_type {
                BinType::Numerical => {
                    ...
                    bins_sorted_f.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
...
fn return_accounted_bins(items: &[(ValueKey, u64)], barn_depth: usize) -> Vec<(ValueKey, u64)> {
    if items.is_empty() || barn_depth == 0 {
        return Vec::new();
    }
    let mut sorted: Vec<(ValueKey, u64)> = items.to_vec();
    sorted.sort_by(|a, b| b.1.cmp(&a.1));

    let mut new_bins_barn: Vec<(ValueKey, u64)> =
        sorted.iter().take(barn_depth).cloned().collect();

    if barn_depth > sorted.len() {
        return new_bins_barn;
    }

    let barn_last = sorted[barn_depth - 1].1;
    let mut iterater = barn_depth;
    while iterater < sorted.len() && sorted[iterater].1 == barn_last {
        new_bins_barn = sorted.iter().take(iterater).cloned().collect();
        iterater += 1;
    }
