//! End-to-end chunked driver (`return_barn`).

use crate::arrow_io::{batch_from_map, concat_same_schema, split_batch_xy};
use crate::preprocess::{BarnacleDepth, PreprocessStream};
use crate::psychic::PsychicBarnacle;
...
/// Chunked pipeline matching `return_barn` / `pb_stream_parquet` (training passes + apply pass).
pub fn return_barn_record_batches(
    batches: &[RecordBatch],
    target: &str,
    cols_to_drop: &[String],
    depth: &BarnacleDepth,
    ...
    let mut pb = PsychicBarnacle::new();
    let mut first = true;
    for b in batches {
        ...
        if first {
            pb.initialize_inputs(&col_graph, target, &column_order)?;
            pb.make_col_combos();
            first = false;
        }
        pb.vals_map_updating(&x_proc, &outcomes)?;
    }
    pb.finish_map();
    ...
    for b in batches {
        ...
        let x_proc = pst.use_map(&table)?;
        let outcomes = nan0(&y);
        let nnm = pb.use_map(&x_proc, &y, &outcomes)?;
...
fn build_output_schema(
    ...
    for name in column_order {
        let col = nnm.get(name).ok_or_else(|| format!("missing column {name} in output"))?;
        let dt = match col {
    ...
    for name in column_order {
        let barn = format!("{name}_barn");
        if nnm.contains_key(&barn) {
            let col = nnm.get(&barn).unwrap();
            let dt = match col {
                ColumnVec::F32(_) => DataType::Float32,
                ColumnVec::Utf8(_) => DataType::Utf8,
            };
            fields.push(Field::new(&barn, dt, false));
        }
    }
    if nnm.contains_key("Actuals") {
        fields.push(Field::new("Actuals", DataType::Float32, false));
    }
    if nnm.contains_key("outcomes_barn") {
        fields.push(Field::new("outcomes_barn", DataType::Float32, false));
    }
...
mod tests {
    ...
    fn pipeline_runs() {
        let b = batch_small();
        let depth = BarnacleDepth::new(4);
        let r = return_barn_record_batches(
            &[b.clone(), b],
            "target",
            &["target".into()],
            &depth,
